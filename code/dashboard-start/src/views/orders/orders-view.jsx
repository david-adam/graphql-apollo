  ...
  render() {
    return (
      <Card
        ...
        ctTableResponsive
        content={
          this.props.allOrdersQuery.loading ? (
            "Loading..."
          ) : (
            <Table striped hover>
              <thead>
              ...
              <tbody>
                {this.props.allOrdersQuery.allOrders &&
                  this.props.allOrdersQuery.allOrders.map((prop, key) => {
                    return (
                      <tr key={key} onClick={this.viewOrder}>
                        ...
                        <td>
                          <button
                            className={`btn btn-${
                              prop.isCompleted ? "danger" : "primary"
                            }`}
                            onClick={() => {
                              prop.isCompleted
                                ? this.props.unFullfillOrderMutation({
                                    variables: { id: prop.id }
                                  })
                                : this.props.fullfillOrderMutation({
                                    variables: { id: prop.id }
                                  });
                            }}
                          >
                            {prop.isCompleted ? "UN" : ""}FULLFILL
                          </button>
                        </td>
                        ...
                        <td>
                          {prop.productQuantityPerOrders.map(
                            (productQuantityPerOrder, index) => (
                              <div key={index}>
                                {productQuantityPerOrder.product.name} x{" "}
                                {productQuantityPerOrder.quantity}
                              </div>
                            )
                          )}
                        </td>
...
const OrderListWithMutations = graphql(FULLFILL_ORDER_MUTATION, {
  name: "fullfillOrderMutation"
})(
